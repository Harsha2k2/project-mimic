"""Model registry and rollout channel assignment primitives."""
...
ROLLOUT_CHANNELS = ("dev", "canary", "prod")
...
class ModelRegistry:
    def __init__(self, *, store: ModelRegistryStore | None = None) -> None:
        self._store = store or InMemoryModelRegistryStore()
        self._versions: dict[str, dict[str, Any]] = {}
        self._channels: dict[str, dict[str, Any] | None] = {channel: None for channel in ROLLOUT_CHANNELS}
        self._restore()
    ...
    def register_version(
        ...
        *,
        ...
        key = self._version_key(cleaned_model_id, cleaned_version)
    ...
    def list_versions(self, *, model_id: str | None = None) -> list[dict[str, Any]]:
    ...
    def promote(self, *, channel: str, model_id: str, version: str) -> dict[str, Any]:
        normalized_channel = channel.strip().lower()
        if normalized_channel not in ROLLOUT_CHANNELS:
            raise ValueError(f"unsupported channel: {channel}")

        key = self._version_key(model_id.strip(), version.strip())
    ...
    @staticmethod
    def _version_key(model_id: str, version: str) -> str:
